<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use App\User;
use App\Friend_User;
use App\PendingRequests;

class FriendsController extends Controller
{
    public function getIndex()
    {

      $not_friends = User::where('id', '!=', Auth::user()->id);
      if (Auth::user()->friends->count()) {
        $not_friends->whereNotIn('id', Auth::user()->friends->modelKeys());
      }
      $not_friends = $not_friends->get();
      
      //$not_friends = Friends::where('friend_1', "!=", Auth::user()->id)->and('friend_2', "!=", Auth::user()->id)->first();
      //$not_friends = Users::addSelect(['not_friends' => ])->get();

      // get all friends
      $friends = Friend_User::where('friend_id', Auth::user()->id)->orWhere('user_id', Auth::user()->id)->get();

      // get all recieved requests
      $recieved_requests = PendingRequests::where('reciever_id', Auth::user()->id)->get();

      // get all sent requests
      $sent_requests = PendingRequests::where('sender_id', Auth::user()->id)->get();

      return view('friends.index')->with('not_friends', $not_friends)->with('friends', $friends)->with('recieved_requests', $recieved_requests)->with('sent_requests', $sent_requests);
    }

public function destroy($id)
{
  $friend = Friend_User::where('friend_id', $id)->orWhere('user_id', $id)->first();

  // if the caller is not one of the users in the friend_user entry
  if ((Auth::user()->id !== $friend->user_id) || (Auth::user()->id !== $friend->friend_id)) {
    return redirect('/friends/index')->with('error',"You cannot delete someone else\'s friends!");
  }

  $friend->delete();

  return redirect('home')->with('success','Friend deleted successfully');
}

public function createRequest($id)
{
  if(Auth::user()->id === $id)
  {
    return redirect('/friends/index')->with('error','cant add yourself');
  }

  $pending_request = new PendingRequests();
  $pending_request->sender_id = Auth::user()->id;
  $pending_request->reciever_id = $id;
  $pending_request->save();

  return redirect('/friends/index')->with('success','Friend added successfully');
}

public function acceptRequest($id)
{
  $pending_request = PendingRequest::where('id', $id)->first();

  // check if the caller is the request reciever
  if (Auth::user()->id !== $pending_request->reciever_id) {
    return redirect('/friends/index')->with('error', 'You cannot accept this request');
  }

  // add entry to friends table
  $friend = new Friend_User();
  $friend->user_id = Auth::user()->id;
  $friend->friend_id = $pending_request->sender_id;
  $friend->save();

  // delete entry from pending requests table
  $pending_request->delete();
}

public function declineRequest($id)
{
  $pending_request = PendingRequest::where('id', $id)->first();

  // check if the caller has permission to delete the request
  if ((Auth::user()->id !== $pending_request->reciever_id) || (Auth::user()->id !== $pending_request->sender_id)) {
    return redirect('/friends/index')->with('error', 'You cannot delete this request');
  }

  $pending_request->delete();

  return redirect('/friends/index')->with('success', 'Request removed!');
}

}